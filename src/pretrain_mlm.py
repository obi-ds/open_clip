...
def main(args):
    ...
    visual_model = TokenReconstructionWrapper(
        ...
        hidden_features=1024,
        output_features_per_lead=(model.visual.scattering_output_size) * model.visual.scattering_signal_length,
    )
    ...
    if args.distributed and not args.horovod:
        if args.use_bn_sync:
            ...
            visual_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(visual_model)
        ddp_args = {}
        if args.ddp_static_graph:
            # this doesn't exist in older PyTorch, arg only added if enabled
            ddp_args['static_graph'] = True
        #model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[device], **ddp_args)
        visual_model = torch.nn.parallel.DistributedDataParallel(visual_model, device_ids=[device], **ddp_args)
    ...
    #data = get_data(args, (preprocess_train, preprocess_val), epoch=start_epoch, tokenizer=None)
    ...
    original_model = visual_model
    if args.torchcompile:
        logging.info('Compiling model...')
        visual_model = torch.compile(original_model)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_one_epoch_mlm(visual_model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=writer)
        completed_epoch = epoch + 1

        if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
            evaluate_mlm(visual_model, data, completed_epoch, args, tb_writer=writer)
