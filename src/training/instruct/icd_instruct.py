...
class MultiTaskInstruct(InstructInterface):
    """
    Make training data from icd codes
    """
    ...
    def process_sample_from_args(self, sample, args):
        ...
        for instruction in instructions:
            input_text, output_text = self.process_instructions(instructions=instruction)
            tokens = self.tokenize_instruction(input_text=input_text, output_text=output_text)
            prompt.append(tokens)
        return torch.cat(prompt)