...
class DiagnosisLabelPredictionTask(object):
    """
    ...
    def __init__(
        ...
        self._diagnosis_instructions = diagnosis_instructions
    ...
    def get_tf_idf(self, encounter_history, label):
        ...
        if self._update_code_counts:
            weights = self.get_dynamic_idf(codes=tf_idf[self._code_column], label=label)
        else:
            weights = tf_idf[self._idf_column]
        tf_idf[self._weights_column] = tf_idf['count'] * weights
        return tf_idf
    ...
    def get_task_instruction(self, prediction_range):
        ...
        task_definition = self._diagnosis_instructions.get_task_definition(time=time)
    ...
    @staticmethod
    def get_weights_for_negative_sampling():
        """
...
class HierarchicalDiagnosisLabelPredictionTask(DiagnosisLabelPredictionTask):
    """
    Make instruction tuning training data from diagnostic codes
    """
    ...
    def __init__(
            self,
            encounter_dataframe_process: EncounterDataframeProcess,
            diagnosis_instructions: HierarchicalDiagnosisLabelPredictionInstructionTemplate,
            code_convert: Union[ICDConvert, PHEConvert],
            negative_code_sampling: NegativeCodeCacheSampling,
            fixed_position_range: bool,
            patient_id_column: str = 'PatientID',
    ...
    def convert_samples_to_instructions(
        ...
        for row in instruction_samples:
            ...
            code_instruct_strings = self._diagnosis_instructions.get_instruction(
