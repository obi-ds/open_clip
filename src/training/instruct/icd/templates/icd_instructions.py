...
from .icd_status_instruction_template import ICDStatusInstructionTemplate, ICDStatusRangeInstructionTemplate
...
class ICDStatusRangeClassificationInstructions(ICDStatusClassificationInstructionsInterface):
    ...
    def get_negative_instruction(
        ...
        if position >= 1 or (position == 0 and self._lock_range):
            if self._lock_range:
                start_time, end_time = np.abs(int(start_range_limit)), np.abs(int(end_range_limit))
            else:
    ...
    def get_position_ranges(self, position: int, start_range_limit: int, end_range_limit):
        pass
