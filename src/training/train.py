...
def mask_input(features, mask_prob=0.4, mask_token=-5000):
    """
    Masks a portion of the input features with a specified probability.

    :param features: Tensor of input features (batch_size, num_features, feature_dim).
    :param mask_prob: Probability of masking each feature.
    :param mask_token: The value to be used for masked features.
    :return: Masked features and mask indices.
    """
    batch_size, num_features, feature_dim = features.size()

    # Create a mask of the same shape as the features
    mask = torch.full((batch_size, num_features), False, dtype=torch.bool, device=features.device)

    # Decide which features to mask
    for i in range(batch_size):
        mask[i] = torch.rand(num_features) < mask_prob

    # Apply the mask
    masked_features = features.clone()
    masked_features[mask] = mask_token

    return masked_features, mask


def masked_mse_loss(masked_input, masked_output, mask=None):
    """
    Calculate Mean Squared Error Loss. If mask is provided, calculate for masked portions only.

    :param masked_input: Original input with masked values.
    :param masked_output: Output from the model for masked input.
    :param mask: (Optional) Boolean tensor indicating masked positions.
    :return: MSE loss.
    """
    if mask is not None:
        # Ensure the mask is boolean and apply it
        mask = mask.bool()
        masked_input = masked_input[mask]
        masked_output = masked_output[mask]

    # Calculate MSE loss
    loss = F.mse_loss(masked_output, masked_input, reduction='mean')
    return loss


def masked_cosine_similarity_loss(masked_input, masked_output, mask=None):
    """
    Calculate Cosine Similarity Loss. If mask is provided, calculate for masked portions only.
    ...
    masked_input = F.normalize(masked_input, p=2, dim=1)
    masked_output = F.normalize(masked_output, p=2, dim=1)

    # Calculate cosine similarity
    cosine_sim = torch.sum(masked_input * masked_output, dim=1)
...
def mask_cls_token(mask):
    batch_size, seq_length = mask.shape
    extra_true = torch.ones((batch_size, 1), dtype=torch.bool, device=mask.device)
    updated_mask = torch.cat((mask, extra_true), dim=1)
    return updated_mask


def train_one_epoch_mlm(visual_model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None):
    device = torch.device(args.device)
    autocast = get_autocast(args.precision)
    input_dtype = get_input_dtype(args.precision)
    ...
    for i, batch in enumerate(dataloader):
        ...
        #masked_images = masked_images.to(device=device, dtype=input_dtype, non_blocking=True)
        ...
        # TODO only implementing accum_freq = 1
        # else:
        #     # First, cache the features without any gradient tracking.
        #     with torch.no_grad():
        #         with autocast():
        ...
        if scaler is not None:
            if args.horovod:
                ...
                scaler.unscale_(optimizer)
                if args.grad_clip_norm is not None:
                    torch.nn.utils.clip_grad_norm_(visual_model.parameters(), args.grad_clip_norm, norm_type=2.0)
                with optimizer.skip_synchronize():
                    scaler.step(optimizer)
            else:
                if args.grad_clip_norm is not None:
...
def evaluate_mlm(model, data, epoch, args, tb_writer=None, tokenizer=None):
    metrics = {}
    if not is_master(args):
        return metrics
    device = torch.device(args.device)
    ...
    if 'val' in data and (args.val_frequency and ((epoch % args.val_frequency) == 0 or epoch == args.epochs)):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                cumulative_reconstruction_mse_loss += latent_mse_loss.cpu().numpy()
                num_samples += batch_size
                if is_master(args) and (i % 100) == 0:
                    logging.info(
                        f"Eval Epoch: {epoch} [{num_samples} / {samples_per_val}]\t"
            ...
            metrics = {
                "val_latent_cosine_loss": cumulative_latent_cosine_loss,
                "val_latent_mse_loss": cumulative_latent_mse_loss,
                "val_reconstruction_mse_loss": cumulative_reconstruction_mse_loss,
                "epoch": epoch,
...
def get_step(data, accum_freq, epoch):
    if 'train' in data:
        dataloader = data['train'].dataloader
        num_batches_per_epoch = dataloader.num_batches // accum_freq
        return num_batches_per_epoch * epoch
    else:
        return None

def log_metrics(metrics, args, tb_writer, epoch, step):
    log_data = {"val/" + name: val for name, val in metrics.items()}
