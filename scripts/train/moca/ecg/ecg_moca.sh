...
# 3. Pre-trained - LoRA - Diagnosis - Random - Fixed - Future
# 2.5e-5 LR seems to be okay, maybe we can try 2e-5 later
# Trial 9 had 128 query tokens, we should do another trial with 64 query tokens - change model config file
# If the performance is similar we can stick with 64 - computationally better - Trial 13
# (I think we can increase batch size back to 100)
# Train without Q-Former? - Trial 12
# Trial 16 - lower LR 1.5e-5
# Final result - Trial 13 (Epoch 150) was the best - LR was 2.5e-5, Qformer tokens was 64
export CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7'
torchrun \
    --nnodes=1 --nproc_per_node=8 --master_addr=localhost --master_port=2120 \
    -m main \
    ...
    --fixed-position-range \
    --future-only \
    --name ecg_moca_pretrained_lora_diagnosis_k_1_random_fixed_future_250_trial_16 \
    --model ecg_moca_biogpt_lora \
    --seed 0



# 4. Pre-trained - LoRA - Labs - Demographics - Diagnosis - Random - Fixed - Past/Future - Multi
export CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7'
torchrun \
    --nnodes=1 --nproc_per_node=8 --master_addr=localhost --master_port=2120 \
    -m main \
    ...
    --training-eval-codes CV_424.4 EM_249 GU_627.2 NS_324.11 CV_416.214 \
    --fixed-position-range \
    --name ecg_moca_pretrained_lora_demographics_diagnosis_ecg_attributes_labs_random_fixed_multi_300_trial_3 \
    --model ecg_moca_biogpt_lora \
    --seed 0
